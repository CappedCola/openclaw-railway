...
// ── Browser Manager (On-demand, Xvfb-backed) ─────────────────────────────────
// Browser is NOT always running. It spins up when the first viewer connects
// via /browser/ws and tears down 30 s after the last viewer leaves.
// With viewers: Chromium runs on a virtual Xvfb display so rendering is real.
// Without viewers: closed completely (zero CPU/memory overhead).
...
const BROWSER_DISPLAY = ':99';
const BROWSER_IDLE_MS = 30_000; // idle teardown delay after last viewer

const browserMgr = {
  viewers: new Set(),   // active WebSocket clients
  xvfbProc: null,      // Xvfb subprocess
  browser: null,        // Playwright browser instance
  page: null,           // active page
  cdpSession: null,     // active CDP session for screencast
  idleTimer: null,
  mode: "agent",        // "agent" (headless) | "interactive" (viewer-driven)
};

async function browserEnsureXvfb() {
  if (browserMgr.xvfbProc) return;
  browserMgr.xvfbProc = childProcess.spawn(
    'Xvfb',
    [BROWSER_DISPLAY, '-screen', '0', `${BROWSER_WIDTH}x${BROWSER_HEIGHT}x24`, '-ac', '-nolisten', 'tcp'],
    { stdio: 'ignore', detached: false },
  );
  browserMgr.xvfbProc.on('exit', (code) => {
    console.log(`[browser] Xvfb exited (${code})`);
    browserMgr.xvfbProc = null;
  });
  await sleep(500); // give Xvfb a moment to open the display
  console.log(`[browser] Xvfb started on ${BROWSER_DISPLAY}`);
}

async function browserGetPage() {
  if (browserMgr.page && !browserMgr.page.isClosed()) return browserMgr.page;

  const hasDisplay = !!browserMgr.xvfbProc;
  const { chromium } = await import('playwright-core');
  const args = ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage'];
  if (!hasDisplay) args.push('--disable-gpu');

  browserMgr.browser = await chromium.launch({
    executablePath: process.env.PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH || '/usr/bin/chromium',
    args,
    headless: !hasDisplay,
    env: hasDisplay ? { ...process.env, DISPLAY: BROWSER_DISPLAY } : undefined,
  });

  const ctx = await browserMgr.browser.newContext({ viewport: { width: BROWSER_WIDTH, height: BROWSER_HEIGHT } });
  browserMgr.page = await ctx.newPage();
  await browserMgr.page.goto('about:blank');
  console.log(`[browser] Chromium launched (${hasDisplay ? `display ${BROWSER_DISPLAY}` : 'headless'})`);
  return browserMgr.page;
}

async function browserStartScreencast(ws) {
  const page = await browserGetPage();
  if (!browserMgr.cdpSession) {
    browserMgr.cdpSession = await page.context().newCDPSession(page);
    await browserMgr.cdpSession.send('Page.startScreencast', {
      format: 'jpeg', quality: 60, maxWidth: BROWSER_WIDTH, maxHeight: BROWSER_HEIGHT,
    });
    browserMgr.cdpSession.on('Page.screencastFrame', async (params) => {
      const msg = JSON.stringify({ type: 'frame', data: params.data, width: BROWSER_WIDTH, height: BROWSER_HEIGHT });
      for (const client of browserMgr.viewers) {
        if (client.readyState === client.OPEN) client.send(msg);
      }
      await browserMgr.cdpSession.send('Page.screencastFrameAck', { sessionId: params.sessionId }).catch(() => {});
    });
    page.on('framenavigated', (frame) => {
      if (frame === page.mainFrame()) {
        const navMsg = JSON.stringify({ type: 'nav', url: page.url() });
        for (const client of browserMgr.viewers) {
          if (client.readyState === client.OPEN) client.send(navMsg);
        }
      }
    });
  }
  browserMgr.viewers.add(ws);
  if (browserMgr.page) {
    ws.send(JSON.stringify({ type: 'nav', url: browserMgr.page.url() }));
    // Send an immediate screenshot so the viewer sees the current state
    // without waiting for the next organic screencast frame (static pages like
    // about:blank won't produce frames on their own after initial load).
    try {
      const { data } = await browserMgr.cdpSession.send('Page.captureScreenshot', {
        format: 'jpeg',
        quality: 60,
      });
      if (ws.readyState === ws.OPEN) {
        ws.send(JSON.stringify({ type: 'frame', data, width: BROWSER_WIDTH, height: BROWSER_HEIGHT }));
      }
    } catch (err) {
      console.warn('[browser] initial screenshot failed:', err.message);
    }
  }
}

async function browserTeardown() {
  clearTimeout(browserMgr.idleTimer);
  browserMgr.idleTimer = null;
  try {
    if (browserMgr.cdpSession) {
      await browserMgr.cdpSession.send('Page.stopScreencast').catch(() => {});
    }
  } catch {}
  browserMgr.cdpSession = null;
  try { await browserMgr.browser?.close(); } catch {}
  browserMgr.browser = null;
  browserMgr.page = null;
  try { browserMgr.xvfbProc?.kill(); } catch {}
  browserMgr.xvfbProc = null;
  console.log('[browser] teardown complete');
}

function browserViewerDisconnected(ws) {
  browserMgr.viewers.delete(ws);
  if (browserMgr.viewers.size === 0) {
    clearTimeout(browserMgr.idleTimer);
    browserMgr.idleTimer = setTimeout(async () => {
      if (browserMgr.viewers.size === 0) {
        console.log('[browser] all viewers gone — tearing down after idle timeout');
...
function createBrowserWebSocketServer(httpServer) {
  ...
  wss.on('connection', async (ws) => {
    const viewerCount = browserMgr.viewers.size + 1;
    console.log(`[browser-ws] viewer connected (${viewerCount} total)`);
    clearTimeout(browserMgr.idleTimer);

    try {
      // First viewer: start Xvfb so the browser gets a real display.
      // If the browser was previously running headless, close it so it
      // relaunches attached to the virtual display.
      if (browserMgr.viewers.size === 0) {
        await browserEnsureXvfb();
        if (browserMgr.browser && !browserMgr.xvfbProc) {
          // shouldn't happen, but guard anyway
        } else if (browserMgr.browser) {
          // browser was headless — restart on display
          try { await browserMgr.browser.close(); } catch {}
          browserMgr.browser = null;
          browserMgr.page = null;
          browserMgr.cdpSession = null;
      ...
      await browserStartScreencast(ws);
    } catch (err) {
      console.error('[browser-ws] startup error:', err.message);
      ws.send(JSON.stringify({ type: 'error', message: `Browser failed to start: ${err.message}` }));
      ws.close();
      return;
    }
    ...
    ws.on('close', () => {
      browserViewerDisconnected(ws);
      console.log(`[browser-ws] viewer disconnected (${browserMgr.viewers.size} remaining)`);
    });

    ws.on('error', (err) => {
      console.warn('[browser-ws] error:', err.message);
    });
  });
...
app.get('/api/browser/status', requireSetupAuth, async (req, res) => {
  ...
  const running = !!(browserMgr.page && !browserMgr.page.isClosed());
  return res.json({
    enabled: true,
    viewers: browserMgr.viewers.size,
    hasDisplay: !!browserMgr.xvfbProc,
    running,
    url: running ? browserMgr.page.url() : null,
  });
});
...
// ── Task Endpoints ─────────────────────────────────────────────────────────
app.get("/api/tasks", requireSetupAuth, async (_req, res) => {
  res.json(await readJsonFile(TASKS_FILE()));
});

app.post("/api/tasks", requireSetupAuth, async (req, res) => {
...
app.put("/api/tasks/:id", requireSetupAuth, async (req, res) => {
  const tasks = await readJsonFile(TASKS_FILE());
...
app.delete("/api/tasks/:id", requireSetupAuth, async (req, res) => {
...
app.get("/api/skills/installed", requireSetupAuth, async (_req, res) => {
  ...
  try {
    ...
    res.json(entries.filter((e) => e.isDirectory()).map((e) => ({ name: e.name })));
  } catch {
    res.json([]);
  }
});

app.post("/api/skills/install", requireSetupAuth, async (req, res) => {
  const { slug } = req.body || {};
  if (!slug || !/^[a-zA-Z0-9_-]+$/.test(slug)) {
    return res.status(400).json({ ok: false, output: "Invalid slug" });
  }
  try {
    await fs.promises.mkdir(path.join(WORKSPACE_DIR, "skills"), { recursive: true });
    const result = await runCmd("npx", ["clawhub@latest", "install", slug], { cwd: WORKSPACE_DIR });
    if (result.code === 0) setImmediate(() => ensureGatewayRunning().catch(() => {}));
    res.json({ ok: result.code === 0, output: result.output });
  } catch (err) {
    res.status(500).json({ ok: false, output: String(err.message) });
  }
});

app.delete("/api/skills/:name", requireSetupAuth, async (req, res) => {
  const { name } = req.params;
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
    return res.status(400).json({ ok: false, error: "Invalid skill name" });
  }
  try {
    await fs.promises.rm(path.join(WORKSPACE_DIR, "skills", name), { recursive: true, force: true });
    res.json({ ok: true });
  } catch (err) {
...
// ── Folders Endpoints ──────────────────────────────────────────────────────
app.get("/api/folders", requireSetupAuth, async (_req, res) => {
  try {
    const entries = await fs.promises.readdir(WORKSPACE_DIR, { withFileTypes: true });
    res.json(entries.filter((e) => e.isDirectory() && !e.name.startsWith(".")).map((e) => ({ name: e.name })));
  } catch {
    res.json([]);
  }
});

app.post("/api/folders", requireSetupAuth, async (req, res) => {
  const { name } = req.body || {};
  if (!name || !/^[a-zA-Z0-9_ -]+$/.test(name)) {
    return res.status(400).json({ ok: false, error: "Invalid folder name" });
  }
  try {
    await fs.promises.mkdir(path.join(WORKSPACE_DIR, name), { recursive: true });
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
...
app.post("/api/schedules", requireSetupAuth, async (req, res) => {
  const schedules = await readJsonFile(SCHEDULES_FILE());
  const schedule = {
    id: String(Date.now()),
    name: req.body.name || "Untitled Schedule",
    description: req.body.description || "",
    cronExpression: req.body.cronExpression || "0 9 * * 1-5",
    enabled: req.body.enabled !== false,
    lastRun: null,
    nextRun: req.body.nextRun || null,
    createdAt: new Date().toISOString(),
  };
...
app.put("/api/schedules/:id", requireSetupAuth, async (req, res) => {
  const schedules = await readJsonFile(SCHEDULES_FILE());
...
app.delete("/api/schedules/:id", requireSetupAuth, async (req, res) => {
  let schedules = await readJsonFile(SCHEDULES_FILE());
  const before = schedules.length;
  schedules = schedules.filter((s) => s.id !== req.params.id);
  if (schedules.length === before) return res.status(404).json({ error: "Schedule not found" });
  await writeJsonFile(SCHEDULES_FILE(), schedules);
  res.json({ ok: true });
});

app.post("/api/schedules/:id/run", requireSetupAuth, async (req, res) => {
  const schedules = await readJsonFile(SCHEDULES_FILE());
  const idx = schedules.findIndex((s) => s.id === req.params.id);
  if (idx === -1) return res.status(404).json({ error: "Schedule not found" });
...
app.get("/api/browser/mode", requireSetupAuth, (_req, res) => {
  res.json({ mode: browserMgr.mode || "agent" });
});

app.post("/api/browser/mode", requireSetupAuth, (req, res) => {
  const { mode } = req.body || {};
  if (mode !== "agent" && mode !== "interactive") {
    return res.status(400).json({ ok: false, error: 'mode must be "agent" or "interactive"' });
  }
  browserMgr.mode = mode;
